...
# from common.pyhelpers.props import typed
# from common.pyhelpers.util import colored, makeList, multiMatch
# from common.pyhelpers.root.util import joinSelection, convertToRootLatex


# class Analysis(UniqueObject, ProcessContainer, AuxContainer):
#     """
#     The analysis class which should be the main object of a physics analysis. It *has*
#     :py:class:`Process`, :py:class:`CMSSetup` and :py:class:`CMSSetupData` instances and provides
#     convenience access to :py:class:`Dataset` instances (which are themselves owned by a
#     :py:class:`CMSSetup` instance). *versions* can be an arbitrary mapping of keys to versions.
#     *name*, *id* and *context* are forwarded to the :py:class:`UniqueObject` constructor. *aux* is
#     forwarded to the :py:class:`AuxContainer` constructor.

#     .. py:attribute:: setups
#        type: UniqueObjectIndex
#        read-only

#        The contained :py:class:`CMSSetup` instances.

#     .. py:attribute:: setupData
#        type: dictionary

#        A mapping of :py:class:`CMSSetup` to :py:class:`CMSSetupData` instances.

#     .. py:attribute:: versions
#        type: dictionary

#        Mapping of a key to a version.
#     """

#     uniquenessContext = "analysis"

#     def __init__(self, name, id, context=None, versions=None, aux=None):
#         UniqueObject.__init__(self, name, id, context=context)
#         ProcessContainer.__init__(self)
#         AuxContainer.__init__(self, aux)

#         # cms setups
#         self._setups = UniqueObjectIndex(CMSSetup)

#         # cms setup data
#         self._setupData = {}

#         # versions, a simple dict
...
#     def addSetup(self, *args, **kwargs):
#         """
#         Adds a setup. Shorthand for ``setups.add()``. See :py:class:`UniqueObjectIndex`.
#         """
#         setup = self.setups.add(*args, **kwargs)

#         # add setup data
#         if setup in self._setupData:
#             raise VerboseError("duplicate setupdata, already present", setup)
#         self._setupData[setup] = CMSSetupData(self, setup)

#         return setup

#     def setup(self, nameOrId):
#         """
#         Returns a setup given by its *nameOrId*.
#         """
#         return self.setups.get(nameOrId)

#     def setupData(self, setup):
#         """
#         Returns the setup data for *setup*.
#         """
#         setup = self.setup(setup)
#         return self._setupData[setup]
...
#         except:
#             raise VerboseError("invalid versions", versions)

#         return versions

#     def addVersion(self, key, version):
#         """
#         Adds a new *version* for a *key*. Overwrites existing version with the same key.
#         """
#         self.versions[key] = version

#     def version(self, key):
#         """
#         Convenience function that returns a version for a specific *key*.
#         """
#         return self.versions[key]

#     def datasets(self, setup, skip=None):
#         """
#         Returns the list of datasets that belongs to *setup* (which can be a name, id or a
#         :py:class:`CMSSetup` instance) and that is assigned to at least one process of this
#         analysis. This convenience function takes into account datasets that should be skipped,
#         configurable by the *skip* argument, and the *skipDatasets* instance member.
#         """
#         setup = self.setup(setup)
#         setupData = self.setupData(setup)

#         if skip is None:
#             skip = []
#         else:
#             skip = makeList(skip)[:]
#         skip += setupData.skipDatasets

#         datasets = []
#         for dataset in setup.datasets:
#             # skip this dataset?
#             if dataset.name in skip or dataset.id in skip \
#                     or multiMatch(dataset.name, [s for s in skip if isinstance(s, basestring)]):
#                 continue

#             for process in dataset.processes:
#                 if self.process(process, deep=True, silent=True):
